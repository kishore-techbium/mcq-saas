...
function downloadPDF() {
  ...
  let html = `
    <div style="padding:20px;font-family:sans-serif">
      <h2 style="text-align:center">${collegeName || ''}</h2>
  ...
  Object.entries(data).forEach(([subject, chapters]) => {
    ...
    Object.entries(chapters).forEach(([chapter, val]) => {
      html += `<p><strong>${chapter}</strong></p>`

      val.subtopics.forEach((s) => {
        html += `<p style="font-size:12px">
          ${s.subtopic} – ${s.accuracy.toFixed(1)}% (${s.attempts} attempts)
        </p>`
      })
    })
  })
